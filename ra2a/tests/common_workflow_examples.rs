...
mod basic_execution {
    ...
    #[derive(Debug, Default)]
    struct TestNoTaskHandler;

    #[async_trait::async_trait]
    impl AgentHandler for TestNoTaskHandler {
        async fn handle_message(
            &self,
            _message: Message,
            _metadata: Option<Object>,
            ...
            Ok(SendMessageResponsePayload::Message(Message {
                message_id: "363422be-b0f9-4692-a24d-278670e7c7f1".to_string(),
                context_id: Some("c295ea44-7543-4f78-b524-7a38915ad6e4".to_string()),
                task_id: None,
                role: Role::unspecified_i32(),
                parts: vec![Part {
                    part: Some(PartBase::Text(
                        "Why did the chicken cross the road? To get to the other side!".to_string(),
                    )),
                }],
                metadata: Some(Object::default()),
                extensions: vec![],
            }))
        }
    }
    ...
    #[tokio::test]
    async fn should_respond_no_task() {
        let agent_builder = AgentBuilder::new(TestNoTaskHandler)
            .with_name("test")
            .with_json_rpc_server("[::]:0".parse().unwrap());
        #[cfg(feature = "grpc")]
        let agent_builder = agent_builder.with_grpc_server("[::]:0".parse().unwrap());
